...
public class DeviceLinux implements DeviceNative {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DeviceLinux.class);
    ...
    /**
     * The timeout for interrupt reads.
     * Termination could take up to this long. So we don't want it to be too long.
     * If it's too short the polling loops too often and data is missed.
     */
    ...
    public void initialise(int unit) {
    	    ...
    	    for (USBDevice device = bus.firstDevice; device != null; device = device.next) {
    	        if (device.descriptor.idVendor == VENDOR_GARMIN &&
    	            ...
    	            if (foundDevices == unit) {
                        garminDevice = device;
                        devicePath = bus.getDirName() + "/" + garminDevice.getFileName();
                        break;
    	            } else {
    	                foundDevices++;
    	            }
        ...
        if (deviceHandle.equals(Pointer.NULL)) {
            throw new DeviceException("usb_open failed, error " + LIBUSB.usb_strerror());
        }
        
        if (LIBUSB.usb_set_configuration(deviceHandle, 1) < 0) {
            throw new DeviceException("usb_set_configuration failed , error " + LIBUSB.usb_strerror() + " - Likely to be either a permission problem, or the device is in use");
        }
        
        if (LIBUSB.usb_claim_interface(deviceHandle, 0) < 0) {
            throw new DeviceException("usb_claim_interface failed, error " + LIBUSB.usb_strerror());
        }
        
        USBInterfaceDescriptor.ByReference altSetting = garminDevice.config.usbInterface.altSetting;
        USBEndpointDescriptor[] endpoints = (USBEndpointDescriptor[]) altSetting.endpoints.toArray(altSetting.numberOfEndpoints);
        for (USBEndpointDescriptor endpoint : endpoints) {
            ...
            case USBEndpointDescriptor.TYPE_BULK:
                if ((endpoint.endpointAddress & USBEndpointDescriptor.DIR_MASK) != 0) {
                    bulkIn = endpoint.endpointAddress & USBEndpointDescriptor.ADDRESS_MASK;
                    LOGGER.debug("Endpoint : bulk in      : " + bulkIn);
                } else {
                    bulkOut = endpoint.endpointAddress & USBEndpointDescriptor.ADDRESS_MASK;
                    LOGGER.debug("Endpoint : bulk out     : " + bulkOut);
                }
                break;
            case USBEndpointDescriptor.TYPE_INTERRUPT:
                if ((endpoint.endpointAddress & USBEndpointDescriptor.DIR_MASK) != 0) {
                    interruptIn = endpoint.endpointAddress & USBEndpointDescriptor.ADDRESS_MASK;
    ...
    public void sendPacket(byte[] packet) {
        int bytesSent = LIBUSB.usb_bulk_write(deviceHandle, bulkOut, packet, packet.length, TIMEOUT_BULK);
    ...
    public byte[] receiveAsync() {
        ...
        while (bytesReceived == ETIMEOUT) {
            ...
            bytesReceived = LIBUSB.usb_interrupt_read(deviceHandle, interruptIn, receiveBuffer, receiveBuffer.length, TIMEOUT_INTERRUPT);
        ...
        while (bytesReceived > 0 && packet.length < expectedPacketLength) {
            ...
            System.arraycopy(receiveBuffer, 0, newPacket, packet.length, bytesReceived);
            packet = newPacket;
